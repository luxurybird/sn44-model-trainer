#!/usr/bin/env python3
"""
Train custom player detection model for ScoreVision.

This script trains a YOLO model for detecting players, goalkeepers, and referees.
"""
...
def train_player_model(
    ...
    if best_model_path.exists():
        export_path = Path(project) / name / "best_player.pt"
...
def main():
    ...
    if args.config:
        with open(args.config, 'r') as f:
            ...
            if 'tasks' in config and 'player' in config['tasks']:
                player_config = config['tasks']['player']
                args.epochs = player_config.get('epochs', args.epochs)
                args.batch_size = player_config.get('batch_size', args.batch_size)
                args.imgsz = player_config.get('imgsz', args.imgsz)
