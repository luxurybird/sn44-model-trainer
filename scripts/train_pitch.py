#!/usr/bin/env python3
"""
Train custom pitch keypoint detection model for ScoreVision.

This script trains a YOLO model for detecting 32 pitch keypoints.
"""
...
def train_pitch_model(
    ...
    # Load model
    model_name = f"{architecture}-pose.pt"  # Use pose model for keypoints
    ...
    if resume:
        ...
        try:
            ...
            # Fallback to regular model if pose model not available
            logger.warning(f"{model_name} not found, using {architecture}.pt")
            model = YOLO(f"{architecture}.pt")
            # Convert to pose model
            model.task = "pose"
    ...
    train_params = {
        ...
        "project": project,
        "name": name,
        "task": "pose",  # Keypoint detection task
        "optimizer": "AdamW",
        "lr0": lr,
    ...
    if best_model_path.exists():
        export_path = Path(project) / name / "best_pitch.pt"
...
def main():
    ...
    if args.config:
        with open(args.config, 'r') as f:
            ...
            if 'tasks' in config and 'pitch' in config['tasks']:
                pitch_config = config['tasks']['pitch']
                args.epochs = pitch_config.get('epochs', args.epochs)
                args.batch_size = pitch_config.get('batch_size', args.batch_size)
                args.imgsz = pitch_config.get('imgsz', args.imgsz)
